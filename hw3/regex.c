...
int regex_match(const char *filename, const char *regex, char ***matches) {
    ...
    for (int i = 0; i < n_matched_lines; ++i) {
        ...
        strncpy(matched_lines[i], lines[i], len -1);
        // if (len < 2) {
        //     continue;
        // }
        // if (matched_lines[i][len-1] == '\n' && matched_lines[i][len-2] =='\n') {
        //     matched_lines[i][len-2] = '\0';
        //     matched_lines[i][len-1] = '\0';
        // }
    }
